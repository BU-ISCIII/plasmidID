...
if [ -f $output_dir/$file_name""_""$cluster_cutoff_input ]; then \
	...
	if [ $cd_hit_command  == "psi-cd-hit.pl" ]; then
		...
		$cd_hit_command -i $(basename $input_file) -o $file_name""_""$cluster_cutoff_input -c $cluster_cutoff -G $global_psi_cd_hit -g 1 -prog $psi_cd_hit_program -circle $is_circle || error ${LINENO} $(basename $0) "PSI-CD-HIT command failed. See $output_dir/logs for more information."
